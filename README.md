...
Алгоритм пытается обнаружить противоречие (например, P и ¬P) в наборе логических формул. Если противоречие найдено, значит, множество формул невыполнимо, то есть оно логически противоречиво.
...
Проверяет, является ли формула противоречием, например:
P ∨ ¬P (в данном случае это всегда истинно).
Проверяет, являются ли два выражения противоположными, например, P и ¬P.


implies_to_or


Преобразует импликацию A → B в эквивалентную форму с использованием дизъюнкции:
A → B эквивалентно ¬A ∨ B.


resolve

Реализует резолюцию двух формул, чтобы попытаться упростить их или найти противоречие. Резолюция — это процесс, который заключается в устранении общих переменных с противоположной логической полярностью.

Алгоритм работает так:

Преобразует импликации в дизъюнкции.
Проверяет, можно ли разрешить противоречие, сравнивая подформулы.
Если находит пары противоположных выражений (например, P и ¬P), создаёт новую упрощённую формулу.


resolve_multiple


Применяет функцию resolve ко всему множеству формул:

Берёт каждую пару формул из множества.


Если найдено новое выражение, добавляет его в список.


Если находит противоречие, завершает процесс.


Этот процесс повторяется до тех пор, пока не перестанут появляться новые формулы.
